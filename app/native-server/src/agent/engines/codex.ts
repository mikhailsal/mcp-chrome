 ...
 *
 * This engine follows the event protocol used by the Codex adapter in other/cweb.
 * It handles item.started, item.delta, item.completed, item.failed, error, and
 * related events, then forwards encoded RealtimeEvent payloads to the sidepanel
 * through AgentStreamManager so the Sidepanel -> Native Server -> Codex CLI ->
 * Sidepanel loop stays intact.
 */
