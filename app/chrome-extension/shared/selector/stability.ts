/**
 * Selector Stability - selector stability scoring helpers.
 */
...
/**
 * Compute a stability score for a selector candidate.
 */
...
/**
 * Attach stability data to a selector candidate.
 */
...
/**
 * Compare two selector candidates by priority.
 * Returns a negative value when `a` should win and a positive value when `b` should win.
 */
export function compareSelectorCandidates(a: SelectorCandidate, b: SelectorCandidate): number {
  // 1. User-specified weight wins first.
  const aw = a.weight ?? 0;
  const bw = b.weight ?? 0;
