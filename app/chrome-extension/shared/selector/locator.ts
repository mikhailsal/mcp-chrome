/**
 * Selector Locator - DOM element resolution from selector candidates.
 * Uses a ranked list of selector candidates to locate a DOM element.
 */
...
export class SelectorLocator {
  ...
  /**
   * Verify that an element matches the provided fingerprint.
   */
  ...
  async locate(
    ...
    if (typeof target.selector === 'string' && target.selector.trim()) {
      ...
      if (ensured) {
        ...
        // Fingerprint verification: if it does not match, keep trying candidates.
        const fingerprintOk =
          !fingerprintToVerify ||
        ...
        // Fingerprint mismatch, keep trying selector candidates.
      }
    }
...
/**
 * Create the Chrome-extension transport.
 */
