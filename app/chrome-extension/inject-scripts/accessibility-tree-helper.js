    ...
    try {
      ...
      if (request && request.action === 'rr_overlay') {
        try {
          ...
          if (!root) {
            ...
            title.textContent = 'Record-Replay Runtime Log';
      ...
      if (request && request.action === 'verifyFingerprint') {
        try {
          ...
          // Validate the fingerprint by comparing the stored value with the current element.
          ...
          // If the stored fingerprint includes an id, the current element must have the same id.
