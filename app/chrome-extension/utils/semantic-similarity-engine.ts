...
async function getCachedModelData(modelUrl: string): Promise<ArrayBuffer> {
  ...
  // 1. Try to load the data from cache first
  ...
  try {
    ...
    // 3. Read the data and store it in cache
    ...
    // If fetching fails, remove any potentially incomplete cache entry
...
export class SemanticSimilarityEngine {
  ...
  private _setupWorker(): void {
    ...
    // Method 1: Chrome extension URL (recommended and most reliable in production)
    try {
    ...
    this.worker.onerror = (error: ErrorEvent) => {
      ...
      if (error.error) {
        // Check whether event.error is available
  ...
  private async _doInitializeWithProgress(
    ...
    // Helper used to report progress
    ...
    try {
      ...
      // Prevent recursion: if already running in an offscreen document, force direct Worker mode
      ...
      if (this.useOffscreen) {
        // Use offscreen mode and delegate progress handling to the offscreen document
        reportProgress('initializing', 15, 'Setting up offscreen document...');
        await this.ensureOffscreenDocument();

        // Send the initialization message to the offscreen document
        ...
        // Ensure the config object is serialized correctly by setting every property explicitly
        const configToSend = {
          modelIdentifier: this.config.modelIdentifier,
          localModelPathPrefix: this.config.localModelPathPrefix,
          ...
          useLocalFiles: Boolean(this.config.useLocalFiles), // Force a boolean value
        ...
        // Use JSON serialization to preserve data integrity
        ...
        // Use direct Worker mode, where real progress updates can be reported
  ...
  private async _doInitialize(): Promise<void> {
    ...
    try {
      ...
      // Prevent recursion: if already in an offscreen document, force direct Worker mode
      if (inOffscreenDocument) {
        this.useOffscreen = false;
        console.log(
          'SemanticSimilarityEngine: Running in offscreen document, using direct Worker mode to prevent recursion',
      ...
      if (this.useOffscreen) {
        ...
        const response = await chrome.runtime.sendMessage({
          target: 'offscreen',
          type: OFFSCREEN_MESSAGE_TYPES.SIMILARITY_ENGINE_INIT,
          config: serializedConfig, // Use the original config without overriding useLocalFiles
        });
        ...
        if (this.config.useLocalFiles) {
          // In WXT, resources from the public directory are served from the runtime root path.
          // Use the model identifier directly and let transformers.js add the /models/ prefix.
        ...
        // Explicitly disable token_type_ids in the tokenizer config for models that do not need them
  ...
  /**
   * Initialization for direct Worker mode with progress callback support
   */
  private async _initializeDirectWorkerWithProgress(
    ...
    // Use the progress callback support available in transformers.js 2.17+
    ...
    try {
      ...
      // Fall back to the standard loading path if progress callbacks are not supported
      console.log(
  ...
  public async warmupModel(): Promise<void> {
    ...
    // More representative warmup texts covering multiple lengths and languages
    const warmupTexts = [
      // Short texts
      'Hello',
      'Hola',
      ...
      'Este es un texto mas largo con varias frases. Ayuda a preparar el modelo para diferentes longitudes de texto.',
    ];

    try {
      // Progressive warmup: individual requests first, then batched requests
      ...
      // Preserve warmup results instead of clearing the cache
  ...
  private async _tokenizeText(text: string | string[]): Promise<TokenizedOutput> {
    ...
    // For a single text, try the cache first
    if (typeof text === 'string') {
      ...
      // Explicitly disable token_type_ids for models that do not require them
      if (!this.config.requiresTokenTypeIds) {
    ...
    // For batches, process directly because they are usually not repeated
    const startTime = performance.now();
    const tokenizerOptions: any = {
      padding: true,
      truncation: true,
  ...
  public async getEmbedding(
    ...
    // Delegate to the offscreen document when offscreen mode is enabled
    if (this.useOffscreen) {
      const response = await chrome.runtime.sendMessage({
        target: 'offscreen',
        type: OFFSCREEN_MESSAGE_TYPES.SIMILARITY_ENGINE_COMPUTE,
  ...
  public async getEmbeddingsBatch(
    ...
    if (this.useOffscreen) {
      ...
      // Return early if everything is already cached
      if (uncachedTexts.length === 0) {
        return results as Float32Array[];
      }

      // Request only uncached texts
      const response = await chrome.runtime.sendMessage({
        target: 'offscreen',
        type: OFFSCREEN_MESSAGE_TYPES.SIMILARITY_ENGINE_BATCH_COMPUTE,
        texts: uncachedTexts,
      ...
      // Put results back into their original positions and cache them
  ...
  public async computeSimilarityBatch(
    ...
    // Preserve the original direct-mode logic
    const simStartTime = performance.now();
  ...
  public cosineSimilarity(vecA: Float32Array, vecB: Float32Array): number {
    ...
    if (this.useSIMD && this.simdMath) {
      try {
        // The SIMD version is async, but this API is synchronous for compatibility
        // Fall back to JavaScript here unless this API is refactored to async
  ...
  // Async SIMD-optimized cosine similarity
