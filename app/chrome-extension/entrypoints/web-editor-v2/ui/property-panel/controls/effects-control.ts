...
// Raw effect that could not be parsed
...
export function createEffectsControl(options: EffectsControlOptions): DesignControl {
  ...
  // Cache effect items per element for the current editing session only.
  // WeakMap is used because:
  // 1. Hidden effects are not written to CSS (enabled=false) but still need to be restorable during the session.
  // 2. WeakMap releases memory automatically when elements are removed, with no manual cleanup required.
  // 3. Only inline styles are read, not computed styles, so the cache exists only to preserve user hide/show actions.
  ...
  if (headerActionsContainer) {
    // Place the + button on the right side of the group header, before the chevron
    headerActionsContainer.insertBefore(addBtn, headerActionsContainer.firstChild);
    disposer.add(() => addBtn.remove());
  } else {
    // Fallback: show the toolbar inside the content area
  ...
  function isEditing(): boolean {
    // Only block refresh while a popover is open or a transaction is active.
    // Avoid overly broad focus checks that could prevent syncing external style changes.
  ...
  function closePopover(opts?: { commit?: boolean; rollback?: boolean }): void {
    ...
    // Sync once after closing so currentItems matches the real inline values.
    // This avoids mismatches after browser normalization or value correction.
    if (wasOpen && !rollback) {
      syncFromTarget(true);
    }
  }
  ...
  // Listen during the document capture phase so outside clicks still close the Effects control
