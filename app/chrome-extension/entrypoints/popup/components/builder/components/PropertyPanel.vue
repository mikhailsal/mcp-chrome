<template>
  ...
  <aside class="property-panel">
    <div v-if="node" class="panel-content">
      ...
      <!-- Property form: render with the NodeSpec-driven form engine. -->
      <PropertyFromSpec
        v-if="node"
      ...
      <div class="form-section">
        ...
        <div class="form-group">
          ...
          <input
            ...
            min="0"
            placeholder="Use the global timeout by default"
          />
        </div>
...
const extractErrors = computed(() => {
  ...
  const errs: string[] = [];
  if (!n.config?.saveAs) errs.push('A saved variable name is required');
...
// Highlight and scroll to the target field
watch(
