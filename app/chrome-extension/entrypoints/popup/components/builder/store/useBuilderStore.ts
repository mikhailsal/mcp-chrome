...
export function useBuilderStore(initial?: FlowV2 | null) {
  ...
  function setNodePosition(id: string, x: number, y: number) {
    ...
    // Do not add this to the history stack; user-triggered actions like connect/add/remove handle that.
  }
  ...
  function onConnect(sourceId: string, targetId: string, label: string = 'default') {
    ...
    // Keep only one outgoing edge per label from the same source node.
    for (let i = edges.length - 1; i >= 0; i--) {
      const e = edges[i];
  ...
  // Fallback layout: layered ordering with barycenter sorting, no external dependency.
  ...
  // Auto layout (prefer ELK):
  // - Dynamically import elkjs to avoid permanent bundle cost.
  ...
  async function layoutAuto() {
    try {
      ...
      try {
        layoutFallback();
        toast('ELK auto-layout is unavailable; using the fallback layout instead', 'warn');
      } catch {}
    }
  }
