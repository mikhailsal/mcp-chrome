...
export class RpcServer {
   ...
   * @description Lists queue items sorted by priority DESC and createdAt ASC.
   */
  private async handleListQueue(params: JsonObject | undefined): Promise<JsonValue> {
    const rawStatus = params?.status;

    // Validate the status allowlist.
   ...
   * @description Cancel a queued item, update run state, and emit a run.canceled event.
   * @note Only queued items can be canceled here; use rr_v3.cancelRun for running or paused runs.
   */
  private async handleCancelQueueItem(params: JsonObject | undefined): Promise<JsonValue> {
    ...
    // 5. Publish run.canceled through the EventsBus to guarantee broadcast.
   ...
   * @description Save or update a flow and perform full structural validation.
   */
  private async handleSaveFlow(params: JsonObject | undefined): Promise<JsonValue> {
    ...
    // Check whether this updates an existing flow using the trimmed ID.
    ...
    // Normalize the flow and pass existingFlow so createdAt can be preserved.
    const flow = this.normalizeFlowSpec(rawFlow, existingFlow);

    // Save to storage. The storage layer performs a second validation pass.
   ...
   * @description Delete a flow after confirming there are no linked triggers or queued runs.
   */
  private async handleDeleteFlow(params: JsonObject | undefined): Promise<JsonValue> {
    ...
    // Check for queued runs. Deleting the flow would break those pending runs.
   ...
   * @description Validate and convert input into a complete FlowV3 structure.
   * @param value Raw input.
   * @param existingFlow Existing flow used to preserve createdAt.
   */
  private normalizeFlowSpec(value: unknown, existingFlow: FlowV3 | null = null): FlowV3 {
    ...
    // Validate variables: each item must be an object with a name field.
  ...
  private normalizeNode(value: unknown, index: number): NodeV3 {
    ...
    // Validate id as non-empty after trimming.
    ...
    // Validate kind as non-empty after trimming.
  ...
  private normalizeEdge(value: unknown, index: number): EdgeV3 {
    ...
    // Validate id or generate one if absent.
    let id: EdgeId;
    if (raw.id === undefined || raw.id === null) {
    ...
    // Validate from as non-empty after trimming.
    ...
    // Validate to as non-empty after trimming.
