 ...
 * @description
 * Provides a recovery-enhanced ExecutionKernel implementation with a `recover()` method.
 * Crash recovery is delegated to RecoveryCoordinator.
 *
 * Other execution methods such as startRun and pauseRun remain unimplemented for now.
 */
...
/**
 * Dependencies for the recovery-enabled kernel.
 */
export interface RecoveryEnabledKernelDeps {
  ...
  /** Event bus. */
  events: EventsBus;
  /** ownerId for the current service worker instance. */
  ownerId: string;
  /** Time source. */
  now?: () => UnixMillis;
  /** Logger. */
 ...
 * Other execution methods remain unimplemented for now.
 */
