 ...
 * @description
 * Implements one-shot scheduling via `chrome.alarms.when`.
 *
 * Behavior:
 * - Each trigger maps to a one-time alarm.
 * - After it fires, the trigger is disabled (`enabled=false`) and uninstalled automatically.
 */
...
export interface OnceTriggerHandlerDeps {
  ...
  /**
   * Optional override for disabling a trigger.
   * If omitted, the handler updates the TriggerStore directly.
   */
...
/**
 * Create a once-trigger handler factory.
 */
...
export function createOnceTriggerHandler(
  ...
  // Create the store lazily to avoid problems in test environments.
  ...
  /**
   * Internal uninstall logic that does not call the external uninstall hook.
   */
  ...
  const onAlarm = (alarm: chrome.alarms.Alarm): void => {
    ...
    void (async () => {
      try {
        ...
        // Check whether the version is still current.
