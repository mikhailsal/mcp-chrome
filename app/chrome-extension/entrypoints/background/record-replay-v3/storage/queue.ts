 ...
 * @description Implements queue CRUD operations and atomic claim behavior
 */
 ...
 * @description Implements queue persistence, including the Phase 3 atomic claim flow
 */
  ...
  return {
    ...
    async recoverOrphanLeases(
      ...
      return withTransaction(RR_V3_STORES.QUEUE, 'readwrite', async (stores) => {
         ...
         * - Reclaim strategy: set status -> queued, clear lease, preserve attempt
         */
        const recoverRunningItems = (): Promise<void> =>
          new Promise<void>((resolve, reject) => {
            ...
            request.onsuccess = () => {
              ...
              // Not an orphan: lease exists and belongs to the current ownerId
              ...
              // Reclaim by removing the lease and changing status to queued
              const { lease: _droppedLease, ...itemWithoutLease } = item;
              const updated: RunQueueItem = {
                ...itemWithoutLease,
                status: 'queued',
         ...
         * @description
         * - Orphan definition: no lease, or lease.ownerId !== currentOwnerId
         * - Adoption strategy: keep status=paused, update lease.ownerId to the new ownerId, and renew the TTL
         */
        const recoverPausedItems = (): Promise<void> =>
          new Promise<void>((resolve, reject) => {
            ...
            request.onsuccess = () => {
              ...
              // Adopt by assigning the lease to the new ownerId and renewing the TTL
              const updated: RunQueueItem = {
                ...item,
                updatedAt: now,
                lease: {
        ...
        // Execute in order: handle running first, then paused
