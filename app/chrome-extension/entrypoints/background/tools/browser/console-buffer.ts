 ...
 *
 * Maintains a rolling buffer per tab and continuously collects console events.
 * The buffer is automatically cleared when a tab navigates to a new hostname so logs from different sites do not mix.
 */
...
/**
 * Extract a safe preview from a CDP RemoteObject and drop objectId to avoid leaks.
 */
function extractArgPreview(arg: unknown): unknown {
  const a = arg as Record<string, unknown>;
  if (!a || typeof a !== 'object') return arg;

  // Keep only safe fields and drop objectId.
...
class ConsoleBuffer {
  ...
  /**
   * Ensure buffer capture has started for the given tab.
   */
  ...
  /**
   * Read buffered content for the given tab.
   */
  ...
  private handleTabUpdated(
    ...
    if (typeof nextUrl === 'string') {
      ...
      // Clear the buffer when the hostname changes.
